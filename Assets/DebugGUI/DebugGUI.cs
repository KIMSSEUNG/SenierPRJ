        ...
        foreach (int group in graphGroupBoxesDrawn)
        {
            ...
            foreach (GraphContainer graph in graphDictionary.Values)
            {
                ...
                if (graph.group == group)
                {
                    ...
                    GUI.Label(new Rect(Screen.width - graphWidth - minMaxXOffset-6, groupOrigin, minMaxXOffset, graphHeight), labelGuiContent, minMaxTextStyle);
                    // Min
                    labelGuiContent.text = graph.min.ToString("F2");
                    GUI.Label(new Rect(Screen.width - graphWidth - minMaxXOffset-6, groupOrigin + graphHeight - textSize.y, minMaxXOffset, graphHeight), labelGuiContent, minMaxTextStyle);

                    GUI.color = graph.color;
                    // Name
                    labelGuiContent.text = graph.name;
                    float xOffset = GUIStyle.none.CalcSize(labelGuiContent).x + 17;

                    graphLabelWidth = Mathf.Max(xOffset, graphLabelWidth, minMaxXOffset);

                    GUI.Label(new Rect(Screen.width - graphWidth - xOffset, yOffset+10, xOffset, graphHeight), labelGuiContent, minMaxTextStyle);
